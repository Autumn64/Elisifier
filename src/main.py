...
def menu_descarga():
    if st.session_state['dstate'] == "downloading":
        ...
        st.session_state['did'] = downloader.obtenerUUID()

        downloader.ejecutarDescarga()
        del downloader
...
def menu_principal():
    ...
    sButton = st.columns([1.5, 1, 1.5])[1].button("Iniciar descarga", disabled=st.session_state['drunning'], on_click=callback, args=[True])
    ...
    if sButton and not any(x in url for x in config['accepted']) and config['block_foreign_urls'] == True:
